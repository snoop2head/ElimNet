...
def sub_create_dataloader(
    ...
    """
    # Data Setup
    train_dataset, val_dataset, test_dataset = sub_get_dataset(
...
def sub_get_dataset(
    ...
    ####################### !Check! 이 부분만 추가됬습니다.! ############################
    # train_dataset 중 30%의 데이터만 랜덤으로 추출하여 활용
    # Valid는 그대로 사용
    import numpy

    n = int(len(train_dataset) * 0.3)
    dataset_subset = torch.utils.data.Subset(
        train_dataset, numpy.random.choice(len(train_dataset), n, replace=False)
    )
    #####################################################################################
    return dataset_subset, val_dataset, test_dataset


def sub_get_dataloader(
